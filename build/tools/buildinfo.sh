...
if [ -n "$TARGET_BUILD_VER" ] ; then
    echo "ro.build.display.id=$TARGET_BUILD_VER"
else
...
if [[ $? == 0 && $TARGET_BUILD_VARIANT == "user" ]] ; then
  ...
  echo "ro.build.user=android-builder"  #$USER
fi
echo "ro.build.host=build-server"  #'host-name'
echo "ro.build.tags=$BUILD_VERSION_TAGS"

if [ -n "$TARGET_BUILD_MODEL" ] ; then
  echo "ro.product.model=$TARGET_BUILD_MODEL"
else
