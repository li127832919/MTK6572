...
static int s62x_probe(struct platform_device *pdev);
static int s62x_remove(struct platform_device *pdev);
/*
...
static struct platform_driver s62x_sensor_driver = {
	.probe      = s62x_probe,
	.remove     = s62x_remove,    
	.driver     = {
		.name  = "msensor",
...
static ssize_t show_layout_value(struct device_driver *ddri, char *buf)
{
        ...
        struct s62x_i2c_data *data = i2c_get_clientdata(client);
#if FORCE_SENSORS_LAYOUT == 0
        int fsl = 2 - LAYOUT_BIT_IS_ZERO(efuse6);
#else
...
static int s62x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if FORCE_SENSORS_LAYOUT == 0
                        if (atomic_read(&data->info) == DEVICE_INFO_M1301) efuse6 = LAYOUT_EFUSE_BIT;
#endif
		...
		if(err = s62x_create_attr(&s62x_sensor_driver.driver)){
...
static int s62x_i2c_remove(struct i2c_client *client)
{
		...
		if(err = s62x_delete_attr(&s62x_sensor_driver.driver)){
...
static int __init s62x_init(void)
{
		...
		if(platform_driver_register(&s62x_sensor_driver))
		{
...
static void __exit s62x_exit(void)
{
		...
		platform_driver_unregister(&s62x_sensor_driver);
