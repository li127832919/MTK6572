...
static int BMA250E_CheckDeviceID(struct i2c_client *client)
{
	...
	if(databuf[0]!=BMA250E_FIXED_DEVID)
	{
...
static int BMA250E_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	/* confirm the setting */
	databuf[0] = 0xff;
	hwmsen_read_block(client, BMA250E_REG_POWER_CTL, databuf, 0x01);
	if (databuf[0] != databuf[1])
	{
		GSE_ERR("fail to set power mode!\n");
		return BMA250E_ERR_SETUP_FAILURE;
	}
...
static int BMA250E_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	/* confirm the setting */
	databuf[0] = 0xff;
	hwmsen_read_block(client, BMA250E_REG_DATA_FORMAT, databuf, 0x01);
	if (databuf[0] != databuf[1])
	{
...
static int BMA250E_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	/* confirm the setting */
	databuf[0] = 0xff;
	hwmsen_read_block(client, BMA250E_REG_BW_RATE, databuf, 0x01);
	if (databuf[0] != databuf[1])
	{
...
static ssize_t store_power_mode_value(struct device_driver *ddri, char *buf, size_t count)
{
	...
	if((data == 0) ||(data == 1)){
		if(hwmsen_read_block(obj->client, BMA250E_REG_POWER_CTL, &reg_value, 0x01))
		{
			GSE_ERR("read power ctl register err!\n");
			return BMA250E_ERR_I2C;
		}
		...
		{
			reg_value &= ~BMA250E_MEASURE_MODE;
		}
		else/*Suspend Mode*/
		{
			reg_value |= BMA250E_MEASURE_MODE;
		...
		if(hwmsen_write_block(obj->client, BMA250E_REG_POWER_CTL, &reg_value, 0x01))
		{
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = BMA250E_SetPowerMode( priv->client, !((bool)atomic_read(&priv->sensor_power)));
				}
			}
			break;
...
static int bma250e_resume(struct i2c_client *client)
{
	...
	if(err = bma250e_init_client(client, 0))
	{
...
static void bma250e_late_resume(struct early_suspend *h)
{
	...
	BMA250E_power(obj->hw, 1);
	if(err = bma250e_init_client(obj->client, 0))
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
...
static int bma250e_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(err = bma250e_create_attr(&bma250e_gsensor_driver.driver))
	{
...
static int bma250e_i2c_remove(struct i2c_client *client)
{
	...
	if(err = bma250e_delete_attr(&bma250e_gsensor_driver.driver))
	{
...
{
	GSE_FUN();
	i2c_register_board_info(1, &i2c_bma250e, 1);
