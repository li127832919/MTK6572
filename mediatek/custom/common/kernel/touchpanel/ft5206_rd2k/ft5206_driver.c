       ...
       unsigned long arg)
{
	...
	if(err)
	{
		printk("ft5206-tpd: access error: %08X, (%2d, %2d)\n", cmd, _IOC_DIR(cmd), _IOC_SIZE(cmd));
		return -EFAULT;
	}
...
static  void tpd_down(int x0, int y0, int p) {
    int x, y;
    x = 240 - x0;
    y = 320 - y0;
...
static  void tpd_up(int x0, int y0,int *count) {
    int x, y;
    x = 240 - x0;
    y = 320 - y0;
...
//	if(point_num == 0) return false;
 ...
 static void tpd_reset()
 {
     ...
     {
          printk("ft5206-kpd: resume I2C transfer error, line: %d\n", __LINE__);
     }
     tpd_up(0,0,0);
     input_sync(tpd->dev);
     TPD_DMESG("ft5206-tpd:TPD wake up done\n");
 }
...
//	msleep(100);
	...
	{
		printk("ft5206-tpd: tpd_misc_device register failed\n");
 ...
 static int tpd_local_init(void)
 {
  ...
  TPD_DMESG("Focaltech FT5206 I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
   ...
   if(i2c_add_driver(&tpd_i2c_driver)!=0)
   	{
  	    printk("ft5206-tpd: unable to add i2c driver.\n");
      	return -1;
    }
    if(tpd_load_status == 0) 
    {
    	printk("ft5206-tpd add error touch panel driver.\n");
...
i2c_smbus_write_i2c_block_data(i2c_client, 0xA5, 1, &data);  //TP enter sleep mode
 ...
 static int __init tpd_driver_init(void) {
	 printk("ft5206-tpd : MediaTek FT5206 touch panel driver init\n");
	   i2c_register_board_info(1, &ft5206_i2c_tpd, 1);
		 if(tpd_driver_add(&tpd_device_driver) < 0)
			 TPD_DMESG("ft5206-tpd: add FT5206 driver failed\n");
	 return 0;
 }
 
 /* should never be called */
 static void __exit tpd_driver_exit(void) {
	 printk("ft5206-tpd : MediaTek FT5206 touch panel driver exit\n");
