...
#if	0
static int tpd_keys_local[TPD_KEY_COUNT] = { KEY_HOME,
                                           KEY_MENU,			
                                           KEY_BACK,			
                                           KEY_SEARCH
                                           };
static int tpd_keys_dim_local[TPD_KEY_COUNT][4] = {{40,  500, 80, 40},
                                                 {120, 500, 80, 40},
                                                 {200, 500, 80, 40},
                                                 {280, 500, 80, 40},
                                                 };
...
retry_upgrade:
	...
	hwPowerDown(MT65XX_POWER_LDO_VGP1,"CTP");
	//ts_power_control(touch_dev, POWER_ON);
	hwPowerOn(MT65XX_POWER_LDO_VGP1,VOL_1800,"CTP");
...
//		touch_dev->cap_info.is_zmt200 = 1;
	...
	if (ts_write_cmd(i2c_client, ZINITIX_INIT_FLASH) != I2C_SUCCESS) {
		printk(KERN_INFO "failed to init flash\n");
		goto fail_upgrade;
	}
...
static int tpd_touchinfo(void)
{
	...
	ts_write_cmd(i2c_client, ZINITIX_CLEAR_INT_STATUS_CMD);
	TPD_DMESG("bt4xx clear int status\n");

	if(tpd_halt) {
		TPD_DMESG("bt4xx suspend halt status...\n");
		return false;
	}

	if (ts_read_data(i2c_client, ZINITIX_POINT_STATUS_REG, (u8 *)&(touch_info), sizeof(struct _ts_zinitix_point_info)) < 0) {
		printk(KERN_INFO "tpd_touchinfo : I2C Error\n");
		return false;
	}
...
static int tpd_detect (struct i2c_client *client, int kind, struct i2c_board_info *info) 
{
	TPD_DEBUG("mtk_tpd: enter bt4xx tpd_detect\n");
...
//	int retry_cnt = 0;
	...
	if (ts_check_need_upgrade(firmware_version, minor_firmware_version, reg_data_version) == true) {
		...
		if (ts_read_data(i2c_client, ZINITIX_FIRMWARE_VERSION, (u8 *)&firmware_version, 2) < 0)
			goto fail_init;
		printk(KERN_INFO "touch chip firmware version = %x\r\n", firmware_version);
		...
		TPD_DMESG("zinitix bt4xx no need to upgrade firmware\n");
	}
#endif
	...
	if(res_x != TPD_RES_MAX_X)
		goto fail_init;
	TPD_DMESG("bt4xx touch x resolutin = %d\r\n", TPD_RES_MAX_X);
	...
	if(res_y != TPD_RES_MAX_Y)
		goto fail_init;
	TPD_DMESG("bt4xx touch y resolutin = %d\r\n", TPD_RES_MAX_Y);
	...
	if (ts_read_data(i2c_client,
		...
		(u8 *)&chip_eeprom_info, 2) < 0)
		goto fail_init;
  	TPD_DMESG("bt4xx touch eeprom info = 0x%04X\r\n", chip_eeprom_info);
 
     if (ts_write_reg(i2c_client, ZINITIX_INT_ENABLE_FLAG, reg_mask) !=0)
		 goto fail_init;
     TPD_DMESG("bt4xx touch ZINITIX_INT_ENABLE_FLAG = 0x%04X\r\n", reg_mask);
	 /* read garbage data */
	 for (i = 0; i < 10; i++) {
		 ts_write_cmd(i2c_client , ZINITIX_CLEAR_INT_STATUS_CMD);
		 udelay(10);
 ...
 {	 
	...
	char data;
	i2c_client = client;
    TPD_DEBUG("mtk_tpd: enter bt4xx tpd_i2c_probe\n");
...
static int __devexit tpd_remove(struct i2c_client *client)
{
   ...
   TPD_DEBUG("mtk_tpd: enter bt4xx tpd_remove\n");
#ifdef TPD_HIGH_SPEED_DMA
	if(CTPI2CDMABuf_va)
	{
...
{
	TPD_DMESG("mtk_tpd: enter bt4xx tpd_local_init\n");
...
static void tpd_resume(struct early_suspend *h)
{
	...
	if(ts_init_touch() == false) {
		printk("tpd_resume : tsp init error\n");
		return;
	}
#else
