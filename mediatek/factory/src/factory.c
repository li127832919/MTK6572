...
//add for saving test report
static const int write_test_report(item_t *items, FILE *fp)
{
...
int convStrtoHex(char*  szStr, unsigned char* pbOutput, int dwMaxOutputLen, int*  pdwOutputLen){
    LOGD("Entry %s\n", __FUNCTION__);
    ...
    if(dwMaxOutputLen < (dwStrLen+1)/2){
        return -1;
    }
    ...
    LOGD(TAG "End of for loop\n");
    *pdwOutputLen = i/2;
    LOGD("Leave %s, pdwOutputLen=%d, i=%d\n", __FUNCTION__, *pdwOutputLen, i);  

    return 0;
}


static int ftm_write_nvram_barcode(char* barcode, PRODUCT_INFO *barcode_struct)
{
    int read_nvram_ready_retry = 0;
    F_ID fid;
    int rec_size = 0;
    ...
    memset(barcode_struct, 0, sizeof(PRODUCT_INFO));

    fid = NVM_GetFileDesc(barcode_lid, &rec_size, &rec_num, false);

    LOGD(TAG "After NVM_GetFileDesc write\n");
    ...
    if(strlen(barcode) > (BARCODE_LEN * 2))
    {
        LOGD(TAG "strlen(barcode) > 128\n");
        return -1;
    }

    ret = convStrtoHex(barcode, barcode_to_write, BARCODE_LEN, &barcode_len);
    ...
    {
        barcode_struct->barcode[i] = barcode_to_write[i];
    }

    for(i = 0; i < barcode_len; i++)
    {
        LOGD(TAG "barcode[%d]=%02x", i, barcode_struct->barcode[i]);
    }

    ret = write(fid.iFileDesc, barcode_struct, rec_num*rec_size);

    LOGD(TAG "After write\n");
    ...
    {
        LOGD(TAG "Write barcode to nvram done\n");
    }
    else
    {
        LOGD(TAG "BackupToBinRegin fail\n");
        return -1;
    }
    sync();
    return 0;
...
int write_barcode(int fd, char* barcode)
{
    ...
    {
        barcode_struct = (PRODUCT_INFO *)malloc(sizeof(PRODUCT_INFO));
        result = ftm_write_nvram_barcode(barcode, barcode_struct);
        free(barcode_struct);
    }
...
int modify_barcode(char *USB_read_buffer)
{
    int ccci_handle = -1;
    int i = 0;
    int result = -1;
    ...
    {
        // barcode points to '=', ++ to get the string after '='
        barcode++;
        LOGD(TAG "%s\n", barcode);
        if(is_support_modem(1))
        {
	...
	while(1){
		...
		else if(read_from_usb > 0)
		{
            ...
            else if(strncmp(USB_read_buffer, WRITE_BARCODE, strlen(WRITE_BARCODE)) == 0)
            {
                ...
                modify_barcode(USB_read_buffer);
            }
			else
			{
               LOGD(TAG "module\n");
