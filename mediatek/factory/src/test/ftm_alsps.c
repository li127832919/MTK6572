...
static int alsps_open(struct lps_priv *lps)
{
    ...
    if (lps->als_fd == -1 && lps->ps_fd == -1) {
#ifdef CUSTOM_KERNEL_ALSPS
        lps->als_fd = open("/dev/als_ps", O_RDONLY);
        if (lps->als_fd < 0) {
            FLPLOGE("Couldn't open '%s' (%s)", "/dev/als_ps", strerror(errno));
            return -1;
        }
        lps->ps_fd = lps->als_fd;
#else //#ifdef CUSTOM_KERNEL_ALSPS

#ifdef CUSTOM_KERNEL_ALS
        lps->als_fd = open("/dev/als", O_RDONLY);
        if (lps->als_fd < 0) {
            FLPLOGE("Couldn't open '%s' (%s)", "/dev/als", strerror(errno));
            return -1;
        }
#endif //#ifdef CUSTOM_KERNEL_ALS

#ifdef CUSTOM_KERNEL_PS
        lps->ps_fd = open("/dev/ps", O_RDONLY);
        if (lps->ps_fd < 0) {
            FLPLOGE("Couldn't open '%s' (%s)", "/dev/ps", strerror(errno));
            return -1;
        }
#endif //#ifdef CUSTOM_KERNEL_PS

#endif //#ifdef CUSTOM_KERNEL_ALSPS
        if (lps->ps_fd != -1)
        {
            retry = 0;
            while ((err = ioctl(lps->ps_fd, ALSPS_SET_PS_MODE, &flags)) && (retry ++ < max_retry))
        ...
        {
            retry = 0;
            while ((err = ioctl(lps->als_fd, ALSPS_SET_ALS_MODE, &flags)) && (retry ++ < max_retry)) 
                usleep(retry_period*1000);
            if (err) {
...
static int alsps_close(struct lps_priv *lps)
{
    ...
    int err = 0;
    if (lps->als_fd != -1) {
        if ((err = ioctl(lps->als_fd, ALSPS_SET_ALS_MODE, &flags))) {
    ...
    if (lps->ps_fd != -1)
    {
        if ((err = ioctl(lps->ps_fd, ALSPS_SET_PS_MODE, &flags))) {
    ...
    if (lps->ps_fd != -1 && lps->ps_fd != lps->als_fd)
    {
        close(lps->ps_fd);
    }
...
static int alsps_update_info(struct lps_priv *lps)
{
    ...
    if (lps->ps_fd == -1 && lps->als_fd == -1) {
        FLPLOGE("invalid fd\n");
        return -EINVAL;
    }
    else
    {
        if ((err = ioctl(lps->ps_fd, ALSPS_GET_PS_RAW_DATA, &ps_dat))) {
            FLPLOGE("read ps  raw: %d(%s)\n", errno, strerror(errno));
            //return err;
        }
        if ((err = ioctl(lps->als_fd, ALSPS_GET_ALS_RAW_DATA, &als_dat))) {
            FLPLOGE("read als raw: %d(%s)\n", errno, strerror(errno));
            //return err;
        }
        if ((err = ioctl(lps->ps_fd, ALSPS_GET_PS_TEST_RESULT, &ps_threshold_dat))) {//yucong add for factory mode test support
            FLPLOGE("get thresheld infr: %d(%s)\n", errno, strerror(errno));
    	ps_threshold_dat = 2;
        } 
        if ((err = ioctl(lps->ps_fd, ALSPS_GET_PS_THRESHOLD_HIGH, &ps_high))) {//yucong add for factory mode test support
            FLPLOGE("get thresheld high infr: %d(%s)\n", errno, strerror(errno));
    	ps_high = 0;
        }
        if ((err = ioctl(lps->ps_fd, ALSPS_GET_PS_THRESHOLD_LOW, &ps_low))) {//yucong add for factory mode test support
