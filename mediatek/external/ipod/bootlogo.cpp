...
void cust_show_battery_capacity_fast_charging(UINT32 capacity);
...
{
    free(tmpLogoAddr);
    XLOGD("[ChargingAnimation %s %d]free tmpLogoAddr  \n",__FUNCTION__,__LINE__);
    munmap(front_fb_addr, fb_size*2);
    close(fb_fd);
}
...
static void show_logo_fb(unsigned int index)
{
    ...
    mt65xx_logo_decompress((void*)inaddr, (void*)tmpLogoAddr, logolen, fb_size);
    ...
    {
        memcpy(fb_addr, tmpLogoAddr, fb_size);
    }
    else if (32 == vinfo.bits_per_pixel) // ARGB8888
    {
        unsigned short src_rgb565 = 0;

        unsigned short *s = (unsigned short *)tmpLogoAddr;
...
void mt65xx_disp_show_battery_capacity(unsigned int capacity)
{
    ...
    if(old_logo_bin > 0) 
    {
     //add by @mtk54040 logo size is  small, show old aniamtion 
        ...
        if (get_fast_charging_state()){
            cust_show_battery_capacity_fast_charging(capacity);
        } else {
...
void cust_show_battery_capacity_fast_charging(UINT32 capacity)
{
    int display_width = vinfo.xres;
    int display_height = vinfo.yres;
    int curr_left = 0;
    int num_width = 0;
    int num_height = 0;
    ...
    if (0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "90", 2) ||
        ...
        display_width = vinfo.yres;
        display_height = vinfo.xres;
    }
    ...
    XLOGD("[ChargingAnimation %s %d]capacity : %d, num_width:%d, num_height:%d, top_margin_height:%d\n", __FUNCTION__,__LINE__, capacity, num_width, num_height, top_margin_height);
    if (capacity <= 0)
    {
         return;
    }     
    ...
    if (capacity >= 100){
        show_logo(39); // battery 100//37+1+1     
    } else {
        charging_animation_index++;
        //background 
         XLOGD("[ChargingAnimation %s %d]show background logo ok \n",__FUNCTION__,__LINE__);
        show_logo(40 + charging_animation_index);   //
        curr_left = (display_width - num_width * 2) >> 1;
        if (capacity >= 10){
            curr_left = (display_width - num_width * 3) >> 1;
            XLOGD("[ChargingAnimation %s %d]capacity = %d, show 2nd num : %d\n",__FUNCTION__,__LINE__, capacity, capacity%10);
            RECT fast_num_1_rect = {curr_left, top_margin_height, curr_left + num_width, top_margin_height + num_height};
            show_animation(46+(capacity/10), fast_num_1_rect, (char*)tmpLogoAddr);
            curr_left += num_width;
        }  
        
        RECT fast_num_2_rect = {curr_left, top_margin_height, curr_left + num_width, top_margin_height + num_height};
        //char fast_num_logo_addr[vinfo.xres *vinfo.yres /10/10*2] = {0x0};
        XLOGD("[ChargingAnimation %s %d]show 1st num \n",__FUNCTION__,__LINE__);
        show_animation(46 + (capacity%10), fast_num_2_rect, (char*)tmpLogoAddr);
        

        curr_left += num_width;
        XLOGD("[ChargingAnimation %s %d]show percent \n",__FUNCTION__,__LINE__);        
        RECT fast_percent_rect = {curr_left, top_margin_height, curr_left + num_width, top_margin_height + num_height};
        show_animation(56, fast_percent_rect, (char*)tmpLogoAddr);
