...
import com.mediatek.common.policy.ICarrierTextExt;
...
public final class MediatekClassFactory {
    ...
    static {
        ...
        opInterfaceMap.put(ICarrierTextExt.class,
                "com.mediatek.op.policy.DefaultCarrierTextExt");
    ...
    public static <T> T createInstance(Class<?> clazz, Object... args) {
        ...
        if (commonInterfaceMap.containsKey(clazz)) {
            ...
            obj = pickerObj;
        } else if (clazz == ISwitch3GPolicy.class) {
            Object policyObj = null;
            try {
                Log.i(TAG, "ISwitch3GPolicy init args length : " + args.length +
                    ", args[0] : " + args[0]);
                if (args.length > 0) {
                    if (args[0].equals(ISwitch3GPolicy.POLICY_OP)) {
                        Class<?> clz = Class.forName("com.mediatek.telephony.Switch3GPolicyOp");
                        if (clz != null)
                            policyObj = clz.newInstance();
                        Log.i(TAG, "ISwitch3GPolicy newInstance (Op) : " + policyObj);
                    } else if (args[0].equals(ISwitch3GPolicy.POLICY_DEFAULT)) {
                        Class<?> clz = Class.forName("com.mediatek.telephony.Switch3GPolicyDefault");
                        if (clz != null)
                            policyObj = clz.newInstance();
                        Log.i(TAG, "ISwitch3GPolicy newInstance (Default) : " + policyObj);
                    }
                }
                if (policyObj == null) {
                    Log.i(TAG, "ISwitch3GPolicy newInstance (ERROR) : create request not mapped to any implementation.");
                }
            } catch (Exception e) {
