...
MINT32 AcdkMain::startPreview(Func_CB prvCb)
{
    ...
    if(mSensorDev == SENSOR_DEV_MAIN || mSensorDev == SENSOR_DEV_SUB) 
    {
        ...
        ACDK_LOGE("SENSOR_CMD_GET_UNSTABLE_DELAY_FRAME_CNT fail(0x%x)",mu4SensorDelay);

        if (err != ACDK_RETURN_NO_ERROR)
        {
            ACDK_LOGE("SENSOR_CMD_GET_UNSTABLE_DELAY_FRAME_CNT fail(0x%x)",err);
            err = ACDK_RETURN_UNKNOWN_ERROR;
            goto startPreviewExit;		
        }
    }
