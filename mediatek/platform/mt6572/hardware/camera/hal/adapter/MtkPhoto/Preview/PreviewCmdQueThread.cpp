...
static void se4500RefillBuffer(vector<IhwScenario::PortQTBufInfo> vDeBufPass1Out)
{
    int32_t j ;
    int32_t bufferSize;
    MUINT8* pBuffer;
    ...
    // copy raw data to pTotalMem
    pBuffer = (MUINT8*)vDeBufPass1Out.at(0).bufInfo.vBufInfo.at(0).u4BufVA;
    bufferSize = vDeBufPass1Out.at(0).bufInfo.vBufInfo.at(0).u4BufSize; 
        
    for( j = (bufferSize/2); j>0 ; j--)
    {
        pBuffer[2*j-2] = pBuffer[j-1];  
        pBuffer[2*j-1] = 0x80 ;
    }
}
/******************************************************************************
...
updateOne()
{
    ...
    //se4500RefillBuffer(vDeBufPass1Out);
