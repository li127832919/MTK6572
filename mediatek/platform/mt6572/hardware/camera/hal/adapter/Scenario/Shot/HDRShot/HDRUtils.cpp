...
EVBracketCapture(void)
{
	...
	if(ret) {
		...
		sched_param param_attr;
		param_attr.sched_priority = 0;
		//pthread_create(&mCaptureIMemThread, NULL, HdrShot::allocateCaptureMemoryTask, this);
		pthread_create(&mCaptureIMemThread, &attr, HdrShot::allocateCaptureMemoryTask, this);
		pthread_setschedparam(mCaptureIMemThread, SCHED_OTHER, &param_attr);
	}
#endif
...
Blending(void)
{
	...
	if(mHdrRound==2) {
		pthread_attr_t attr = {0, NULL, 1024 * 1024, 4096, SCHED_OTHER, -20};
		pthread_create(&mNormalJpegThread, &attr, HdrShot::createNormalJpegImgTask, this);
	}
	#endif
	...
	if(mTestMode) {
	    ...
	    for(int i=0; i<mu4OutputFrameNum; i++) {
			...
			if(i==1) {
				...
				sched_param param_attr;
		        param_attr.sched_priority = 0;
				pthread_create(&mProcessIMemThread, &attr, HdrShot::allocateProcessMemoryTask, this);
				pthread_setschedparam(mProcessIMemThread, SCHED_OTHER, &param_attr);
			}
#endif
			char yuvfile[128];
...
MVOID*
HdrShot::
doHDRAlgoTask(MVOID* arg)
{
	FUNCTION_LOG_START;
	MBOOL 	ret = MTRUE;
	prctl(PR_SET_NAME,"Hdr doHDRAlgoTask", 0, 0, 0);
	...
	if(self->mHdrRoundTotal == 1) {
		if(!self->mfgIsForceBreak) {
		ret = ret
			//single run
			&& self->configureForSingleRun()
			&& self->EVBracketCapture()
			&& self->ImageRegistratoin()
			&& self->WeightingMapGeneration()
			&& self->Blending()
			;
		}
	} else {
		...
		ret = ret
			//first run
			&& self->configureForFirstRun()
			&& self->EVBracketCapture()
			&& self->ImageRegistratoin()
			&& self->WeightingMapGeneration()
			&& self->Blending()
			//second run
			&& self->configureForSecondRun()
			&& self->WeightingMapGeneration()
			&& self->Blending()
			;
		}
...
createNormalJpegImgTask(MVOID* arg)
{
	FUNCTION_LOG_START;
	MBOOL 	ret = MTRUE;
	prctl(PR_SET_NAME,"Hdr jpegImgTask", 0, 0, 0);
...
allocateCaptureMemoryTask(MVOID* arg)
{
	...
	prctl(PR_SET_NAME,"Hdr allocateCaptureMemoryTask", 0, 0, 0);
	//non-real-time policy
	SetThreadProp(SCHED_OTHER, -20);
...
allocateProcessMemoryTask(MVOID* arg)
{
	...
	prctl(PR_SET_NAME,"Hdr allocateProcessMemoryTask", 0, 0, 0);
	//non-real-time policy
	SetThreadProp(SCHED_OTHER, -20);
...
saveNormalJpeg(MVOID *arg)
{
	...
	pthread_create(&mSaveJpegThread, &attr, HdrShot::saveFileTask, pFileInfo);
    delete [] puExifHeaderBuf;
	#endif
...
saveFileTask(MVOID* arg)
{
	...
	prctl(PR_SET_NAME,"Hdr saveFileTask", 0, 0, 0);
