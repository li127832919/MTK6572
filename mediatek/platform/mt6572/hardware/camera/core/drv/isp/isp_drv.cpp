...
//-----------------------------------------------------------------------------
MBOOL IspDrv::forcetwocore(void)
{
    MINT32 Ret;
    ...
    //
    LOG_DBG("ISP FORCE TWO CORE, mfd(0x%08x)", mFd);
    //
    Ret = ioctl(mFd,ISP_IOC_FORCE_TWO_CORE,NULL);
    if(Ret < 0)
    {
        LOG_ERR("ISP_IOC_FORCE_TWO_CORE fail(%d)", Ret);
        LOG_DBG(" -");
        return MFALSE;
    }
...
{   //move filled Hw buffer to sw the end of list
    ...
    for(MUINT32 i=0;i<count;i++) 
    {
        ...
        if (checkGreySensor()) {
            convertGreyData(buff.base_vAddr, buff.size, MFALSE);
        }
...
MBOOL IspDrv::checkGreySensor(void)
{
    MUINT32 isGreySensor = 0;

    ioctl(mFd,ISP_IOC_CHECK_GREY, &isGreySensor);

    return (isGreySensor == 1)?MTRUE:MFALSE; 
}

MBOOL greySensorCapture = MFALSE;
MBOOL IspDrv::convertGreyData(unsigned int vAddr, unsigned int size, MBOOL capture) {
    unsigned char * pYSrc = (unsigned char *)vAddr;
    unsigned short * pYDst = (unsigned short *)vAddr;
    int32_t yuvSize = size/2;
    int32_t index = 0;

    if (greySensorCapture == MTRUE) {
        return MTRUE;
    }
    
    if ((pYSrc != NULL) && (yuvSize > 0)) {
        for (int32_t idx = yuvSize-1; idx >= 0; idx--){
            *(pYDst + idx) = *(pYSrc + idx) + 0x8000;
        }
    }
...
MBOOL IspDrv::captureGreySensor(MBOOL capture) {
    greySensorCapture = capture;
    return MTRUE;
}
