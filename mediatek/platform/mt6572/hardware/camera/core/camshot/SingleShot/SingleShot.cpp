...
createSensorRawImg(SensorParam const & rSensorParam, ImgBufInfo const & rRawImgBufInfo)
{
    ...
    mpCamIOPipe->enableNotifyMsg(ECamPipe_NOTIFY_MSG_SOF|ECamPipe_NOTIFY_MSG_EOF|ECamPipe_NOTIFY_MSG_FLASHFIRE); 
    ...
    IspDrv* ispDrv = IspDrv::createInstance();
    if ((ispDrv != NULL) && ispDrv->checkGreySensor()){
        ispDrv->captureGreySensor(MTRUE);
    }
    ...
    //IspDrv* ispDrv = IspDrv::createInstance();
    if ((ispDrv != NULL) && ispDrv->checkGreySensor()){
        ispDrv->captureGreySensor(MFALSE);
        ispDrv->convertGreyData(rRawImgBufInfo.u4BufVA, rRawImgBufInfo.u4BufSize, MTRUE);
    }
    ...
    if (NULL != pSingleShot) 
    {
        switch( msg.msgType )
        {
            ...
            case NSCamPipe::ECamPipe_NOTIFY_MSG_EOF:
                ...
                break;
            case NSCamPipe::ECamPipe_NOTIFY_MSG_FLASHFIRE:
                pSingleShot->fgFireFlash(msg.ext1); 
                break;
            default:
                break;
        }
...
MBOOL
SingleShot::
fgFireFlash(MUINT32 u4deviceID)
{
    NS3A::Hal3ABase *p3AObj = Hal3ABase::createInstance(u4deviceID); 
    p3AObj->sendCommand(ECmd_FlashFire, 0);
    p3AObj->destroyInstance();
    return MTRUE;
}
