...
init(MINT32 i4SensorDev)
{
    ...
    m_pSensorHal->sendCommand((halSensorDev_e)m_i4SensorDev, SENSOR_CMD_GET_SENSOR_ID, (MINT32)&i4CurrSensorId, 0, 0);
    ...
    MY_LOG("[%s] currLensId(0x%08x),dummylens(%d), i4CurrSensorId(0x%08x)", __FUNCTION__, i4CurrLensId, m_bIsdummylens, i4CurrSensorId);
    m_imageXS = 320;
    m_imageYS = 240;
...
MBOOL Hal3AYuv::sendCommand(ECmd_T const eCmd, MINT32 const i4Arg)
{
    ...
    else if (eCmd == ECmd_FlashFire)
    {
        if (m_TrigFlashFire == MTRUE)
        {
            MY_LOG("ECmd_FlashFire: shotMode = %d\n", m_rParam.u4ShotMode);
...
MBOOL Hal3AYuv::set3AEXIFInfo(IBaseCamExif *pIBaseCamExif) const
{
    ...
    rEXIFInfo.u4RealISOValue = mSensorInfo.RealISOValue;
...
//    ai4Zone[0] = 0; ai4Zone[2] = m_imageXS-1;
//    ai4Zone[1] = 0; ai4Zone[3] = m_imageYS-1;
    ai4Zone[4] = m_imageXS;
    ai4Zone[5] = m_imageYS;
...
MINT32 Hal3AYuv::setAFLampOnOff(MBOOL bOnOff)
{
    ...
    {
        if (m_rParam.u4StrobeMode != FLASHLIGHT_TORCH && m_rParam.u4CamMode != eAppMode_VideoMode)
        {
