...
start()
{
    ...
    //  force dual core
    mpCamIOPipe->sendCommand(NSImageio::NSIspio::EPIPECmd_FORCE_TWO_CORE,0,0,0); 
    //
    ...
    if (mrSensorPortInfo.fgBypassDelay == MFALSE) 
    {
        ...
        u4TotalDelayFrame += u4DelayFrame;
...
skipFrame(MUINT32 const u4SkipCount)
{
    ...
    for (MUINT32 i = 0; i < u4SkipCount ; i++)
    {
        ...
        if( u4SkipCount == 1 )
        {
            handleNotifyCallback( ECamPipe_NOTIFY_MSG_FLASHFIRE, mrSensorPortInfo.u4DeviceID, 0 );
        }
