    ...
    do
    {
        ...
        do
        {
            ...
            if(mAudioBTCVSDControl->BT_SCO_isWideBand())
            {
               ...
               ALOGV("WriteDataToBTSCOHW, do mSBC encode outsize=%d, consumed size=%d, bytes=%d",outsize,insize,bytes);
            }
            else
#endif
            {
               ...
               ALOGV("WriteDataToBTSCOHW, do CVSD encode outsize=%d, consumed size=%d, bytes=%d",outsize,insize,bytes);
            }
...
size_t AudioMTKStreamOut::WriteDataToAudioHW(const void *buffer, size_t bytes)
{
    ...
    if (current_device & AUDIO_DEVICE_OUT_SPEAKER)
    {
        ...
        {
            if(mVibSpkState == VIBSPK_GEN_IDLE)
            {
                mVibSpkState = VIBSPK_GEN_RUNNING;
        ...
        {
            if(mVibSpkState == VIBSPK_GEN_RUNNING)
            {
                mVibSpkState = VIBSPK_GEN_STOPPING;
        ...
        if(mVibSpkState != VIBSPK_GEN_IDLE)
        {
            ...
            //int16_t dAudioGain = 0;
            uint32 dSampleCount = bytes >> 1;
            int32_t pAudioOut;
            int16_t *pAudioData = (int16_t *)outbuf_vibspk;
            int16_t *pToneData  = (int16_t *)outbuf_tone;
            while (dSampleCount)
            {
                //*pAudioData = (((*pAudioData) * dAudioGain) >> 15);
                pAudioOut = ( (int32_t)(*pAudioData) * (int32_t)dAudioGain ) >> 15;
                *pAudioData++ = (int16_t)pAudioOut + *pToneData;
                pToneData++;
                dSampleCount--;
            }
        }
