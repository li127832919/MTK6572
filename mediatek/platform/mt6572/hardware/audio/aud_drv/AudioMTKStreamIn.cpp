...
#ifdef MTK_HFP1_6_SUPPORT
                if ( mAudioBTCVSDControl->BT_SCO_isWideBand() )
	               mBliSrc->init(DAIBT_WB_SAMPLE_RATE, 1, mAttribute.mSampleRate, num_channel);
	            else
#endif
...
#if DAI_MEMIF_SUPPORT || defined(BT_SW_CVSD)
#ifdef MTK_HFP1_6_SUPPORT
   if (mStreamInClient->mMemDataType == AudioDigitalType::MEM_DAI) {    
      if (mAudioBTCVSDControl->BT_SCO_isWideBand() && (mAttribute.mSampleRate!=DAIBT_WB_SAMPLE_RATE) )
         return true;
      if ( !mAudioBTCVSDControl->BT_SCO_isWideBand() && (mAttribute.mSampleRate!=DAIBT_SAMPLE_RATE) )
         return true;
      return false;
	}
#else
...
#ifdef MTK_DUAL_MIC_SUPPORT
    //DMNR parameters
#if defined(MTK_HANDSFREE_DMNR_SUPPORT) && defined(MTK_ASR_SUPPORT)
    if (mAttribute.mSource == AUDIO_SOURCE_VOICE_RECOGNITION)
    {
        ALOGD("enable ASR");
...
#ifdef MTK_BESRECORD_2_0_SUPPORT
