...
#define POLLING_TIME 		100000    //0.1ms

//clear ACCDET IRQ in accdet register
static inline void clear_accdet_interrupt(void)
{
	unsigned long long timer_pos,timer_pre = 0; 

	//it is safe by using polling to adjust when to clear IRQ_CLR_BIT
	pmic_pwrap_write(ACCDET_IRQ_STS, (ACCDET_IRQ_CLR));
	//pmic_pwrap_write(INT_CON_ACCDET_CLR, (RG_ACCDET_IRQ_CLR));

	ACCDET_DEBUG("[Accdet]## before clear_accdet_interrupt: ACCDET_IRQ_STS = 0x%x\n", pmic_pwrap_read(ACCDET_IRQ_STS));
	
	timer_pre = sched_clock();
	while(pmic_pwrap_read(ACCDET_IRQ_STS) & ACCDET_IRQ)
	{
				timer_pos = sched_clock();
				if(timer_pos - timer_pre >= POLLING_TIME)
				{
					ACCDET_DEBUG("[Accdet]clear_accdet_interrupt over time , polling time = %d\n", timer_pos - timer_pre);
                                        break;
				}
	
	}
	
	ACCDET_DEBUG("[Accdet]after clear_accdet_interrupt: ACCDET_IRQ_STS = 0x%x , polling time=%d:%d:%d\n", pmic_pwrap_read(ACCDET_IRQ_STS),timer_pos ,timer_pre,timer_pos -timer_pre);
}
...
ACCDET_DEBUG("[Accdet_init] TOP_CKPDN=0x%x!\n", pmic_pwrap_read(TOP_CKPDN));
...
static int dump_register(void)
{
	...
	ACCDET_DEBUG("[0x108] = 0x%x\n", pmic_pwrap_read(0x108));
