...
static int UnderflowCnt = 0; // Yi-Lung: reduce underflow log, print it every 5 times continuously.
...
static bool CheckSize(uint32 size, uint32 base, uint end)
{
    if((size) == 0 || size < base || size >= end){
        PRINTK_AUDDRV("[Auddrv][Warn]CheckSize size = %p, base = %p, end = %p",size, base, end);
        return true;
    }
    return false;
}
...
void Auddrv_DL_Interrupt_Handler(void)  // irq1 ISR handler
{
    ...
    spin_lock_irqsave(&auddrv_irqstatus_lock, flags);
    Afe_dac_con0_temp = Afe_Get_Reg( AFE_DAC_CON0);
    if(Afe_dac_con0_temp&0x2)
       DL1_flag = true;
    if(	Afe_dac_con0_temp&0x4)
       DL2_flag = true;
    
    while(DL1_flag||DL2_flag)
    {
        if(DL1_flag)
        {
        ...
        {
            Afe_Block = &(AFE_dL2_Control_context.rBlock);
        ...
        */
        ...
        if(Afe_Block->u4DataRemained < Afe_consumed_bytes || Afe_Block->u4DataRemained <= 0 ||Afe_Block->u4DataRemained  > Afe_Block->u4BufferSize || AudIrqReset)
        {
            ...
            UnderflowCnt++;
            if(UnderflowCnt % 5 == 1)
                PRINTK_AUDDRV("+DL_Handling underflow ReadIdx:%x WriteIdx:%x, DataRemained:%x, Afe_consumed_bytes:%x HW_memory_index = 0x%x UnderFlow Cnt:%d IRQ_CNT:0x%x\n",
                    Afe_Block->u4DMAReadIdx,Afe_Block->u4WriteIdx,Afe_Block->u4DataRemained,Afe_consumed_bytes,HW_memory_index, UnderflowCnt, Afe_Get_Reg(AFE_IRQ_CNT1));
