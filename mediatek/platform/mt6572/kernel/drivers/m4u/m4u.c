...
#if 1 // def MTK_M4U_EXT_PAGE_TABLE 
/* add for ovl: 
...
{
    int u4PTEntry = PT_TOTAL_ENTRY_NUM;
    ...
    {
        u4PTEntry = (1*1024*1024);
    }
    printk("[lcm] mode:%d, entry:%d \n", DISP_IsDSIVdoMode(), u4PTEntry);

    pPT_nonsec= dma_alloc_coherent(NULL, u4PTEntry * sizeof(unsigned int), &pt_pa_nonsec, GFP_KERNEL);
    if(!pPT_nonsec)
    {
    ...
    if((pt_pa_nonsec&M4U_PAGE_TABLE_ALIGN)!=0)
    {
        ...
        dma_free_coherent(NULL, u4PTEntry * sizeof(unsigned int), pPT_nonsec, pt_pa_nonsec);
        tmp = (unsigned int)dma_alloc_coherent(NULL, u4PTEntry * sizeof(unsigned int)+M4U_PAGE_TABLE_ALIGN, &pt_pa_nonsec, GFP_KERNEL);
        if(!tmp)
        {
            M4UMSG("dma_alloc_coherent error!  dma memory not available.\n");
            return false;
    ...
    memset((void*)pPT_nonsec, 0, u4PTEntry * sizeof(unsigned int));
    ...
    pPT_sec= dma_alloc_coherent(NULL, u4PTEntry * sizeof(unsigned int), &pt_pa_sec, GFP_KERNEL);
    if(!pPT_sec)
    {
    ...
    if((pt_pa_sec&M4U_PAGE_TABLE_ALIGN)!=0)
    {
        unsigned int tmp;
        M4UMSG("dma_alloc_coherent memory not align. PageTablePA=0x%x we will try again \n", pt_pa_sec);
        dma_free_coherent(NULL, u4PTEntry * sizeof(unsigned int), pPT_sec, pt_pa_sec);
        tmp = (unsigned int)dma_alloc_coherent(NULL, u4PTEntry * sizeof(unsigned int)+M4U_PAGE_TABLE_ALIGN, &pt_pa_sec, GFP_KERNEL);
        if(!tmp)
        {
            M4UMSG("dma_alloc_coherent error!  dma memory not available.\n");
            return false;
    ...
    memset((void*)pPT_sec, 0, u4PTEntry * sizeof(unsigned int));
    //======= alloc pagetable done=======================
#endif
