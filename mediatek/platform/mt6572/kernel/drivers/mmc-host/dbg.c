...
int msdc_print_all_msdc_send_stop=0;
...
static int msdc_debug_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    p += sprintf(p, "-> Clk src(%x) sclk(%x) mclk(%x)\n", mtk_msdc_host[0]->hclk, mtk_msdc_host[0]->sclk, mtk_msdc_host[0]->mclk );
    ...
    p += sprintf(p, "\nMSDC1 Regs\n");
    p += sprintf(p, "-> Clk SRC Reg: %x\n",sdr_read32(0xF0000000));
    p += sprintf(p, "-> Clk src(%x) sclk(%x) mclk(%x)\n", mtk_msdc_host[1]->hclk, mtk_msdc_host[1]->sclk, mtk_msdc_host[1]->mclk );
...
static int msdc_debug_proc_write(struct file *file, const char *buf, unsigned long count, void *data)
{
    ...
    if(cmd == SD_TOOL_ZONE) {
        ...
        if ( zone & DBG_EVT_REP_STOP ) {
            ...
            msdc_print_all_msdc_send_stop=1;
        }
