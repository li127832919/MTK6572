...
int mt_irq_mask_all(struct mtk_irq_mask *mask)
{
    ...
    if (mask) {
        ...
        if(!(mask->mask0 & 0xFFFF0000))
        {
            printk(KERN_CRIT "[Error] mt_irq_mask_all, all ppi are masked\n");
        }
...
int mt_irq_mask_restore(struct mtk_irq_mask *mask)
{
    ...
    if (mask->header != IRQ_MASK_HEADER) {
        printk(KERN_CRIT "mask->header=0x%08x, != IRQ_MASK_HEADER\n", mask->header);
        return -1;
    }
    if (mask->footer != IRQ_MASK_FOOTER) {
        printk(KERN_CRIT "mask->footer=0x%08x, != IRQ_MASK_FOOTER\n", mask->footer);
        return -1;
    }
    ...
    if(!(mask->mask0 & 0xFFFF0000))
    {
        printk(KERN_CRIT "[Error] mt_irq_mask_restore, all ppi are masked\n");
    }
