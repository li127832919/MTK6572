...
#define NR_CONNSYSDMA_CHANNEL     (0)
#define NR_PDMA_CHANNEL           (0)
#define NR_VFFDMA_CHANNEL         (0)
#define GDMA_START                (0)
...
int mt65xx_req_gdma(DMA_ISR_CALLBACK cb, void *data)
{
    ...
    if (i < NR_GDMA_CHANNEL) {
        ...
        enable_clock(MT_CG_APDMA_SW_CG, "GDMA 1");
...
int mt65xx_free_gdma(int channel)
{
    ...
    //}
    disable_clock(MT_CG_APDMA_SW_CG, "GDMA 1");
    spin_unlock(&dma_alloc_lock);
...
void mt_reset_dma(const unsigned int iChannel)
{
    ...
    /* to disable confg error message */
    conf.dinc = DMA_TRUE;
    conf.sinc = DMA_TRUE;
