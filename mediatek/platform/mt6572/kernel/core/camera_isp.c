...
int kdCheckGreySensor = 0;

//force two core on
extern void force_two_core();
    ...
    unsigned long   Param)
{
    ...
    switch(Cmd)
    {
        ...
        case ISP_IOC_CHECK_GREY:
        {
            if(copy_from_user(&CheckGreySensor, (void*)Param, sizeof(MUINT32)) == 0)
            {
                CheckGreySensor = kdCheckGreySensor;
                if(copy_to_user((void*)Param, &CheckGreySensor, sizeof(MUINT32)) != 0)
                {
                    LOG_ERR("copy_to_user failed");
                    Ret = -EFAULT;
                }
