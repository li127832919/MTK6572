...
static bool snand_read_oob_raw(struct nand_chip *chip, u32 page_addr, u32 length, u8 * buf)
{
    ...
    while (length > 0)
    {
        ...
        if (!snand_ready_for_read(chip, page_addr, col_addr, false, NULL))
            return false;
        if (!snand_mcu_read_data(buf, length, false))
            return false;
        NFI_CLN_REG16(NFI_CON_REG16, CON_NFI_BRD);
        sector++;
    ...
    switch (command)
    {
        ...
        case NAND_CMD_PAGEPROG:
            if (g_kCMD.pDataBuf || (0xFF != g_kCMD.au1OOB[0]))
            {
              ...
              snand_exec_write_page(nand, g_kCMD.u4RowAddr, nand->writesize, pDataBuf, g_kCMD.au1OOB);
        ...
        case NAND_CMD_ERASE1:
            snand_reset_con();
            snand_set_mode(CNFG_OP_ERASE);
            snand_set_command(NAND_CMD_ERASE1);
            snand_set_address(0, page_addr, 0, devinfo.addr_cycle - 2);
            break;

        case NAND_CMD_ERASE2:
            snand_set_command(NAND_CMD_ERASE2);
        ...
        case NAND_CMD_STATUS:
            NFI_CLN_REG16(NFI_CNFG_REG16, CNFG_BYTE_RW);
            snand_reset_con();
            snand_set_mode(CNFG_OP_SRD);
            snand_set_command(NAND_CMD_STATUS);
