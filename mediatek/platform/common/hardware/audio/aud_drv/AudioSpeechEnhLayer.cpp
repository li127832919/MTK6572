...
void SPELayer::FlushBufferQ(void)
{
    ...
    ALOGD("FlushBufferQ mULOutBufferQ size=%d,mULInBufferQ.size=%d,mDLOutBufferQ.size()=%d,mDLInBufferQ.size()=%d,mDLDelayBufferQ.size()=%d",mULOutBufferQ.size(),mULInBufferQ.size(),mDLOutBufferQ.size(),mDLInBufferQ.size(),mDLDelayBufferQ.size());
...
bool    SPELayer::Start(SPE_MODE mode)  //for VOIP, both uplink/downlink
{
	...
	mSph_Enh_ctrl.Device_mode = ROUTE_SPEAKER;  //force using speaker mode device setting
