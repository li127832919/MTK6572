...
static jfieldID field_mHandsfreeAgRfcommChannel;
static jfieldID field_mHeadsetAgRfcommChannel;
static jfieldID field_mHfpVersion;
    /* out */
static jfieldID field_mTimeoutRemainingMs; /* out */
...
static void classInitNative(JNIEnv* env, jclass clazz) {
    ...
    GET_FIELD_ID(field_mHfpVersion, clazz, "mHfpVersion", "I");
...
static void initializeNativeDataNative(JNIEnv* env, jobject object) {
    ...
    if (field_mHfpVersion){
        nat->version = env->GetIntField(object, field_mHfpVersion);
    }else{
        nat->version = 0x0105;
    }
    ALOGV("[BT][HFG]hfpVersion = %d.", nat->version);    
    return;
...
static jboolean setUpListeningSocketsNative(JNIEnv* env, jobject object) {
    ...
    // Register context
    #if defined(EXT_DYNAMIC_LOADING)
    if( BT_STATUS_SUCCESS ==  nat->hfg_api.hfg_register(nat->pContext[HSP_CONTEXT], NULL, nat->hfgsock, /*nat->hfgservsock,*/ KAL_TRUE, nat->version) )
    #else
    if( BT_STATUS_SUCCESS ==  btmtk_hfg_register(nat->pContext[HSP_CONTEXT], NULL, nat->hfgsock, /*nat->hfgservsock,*/ KAL_TRUE, nat->version) )
    #endif
    {
        ALOGI("[BT][HFG]pContext[HSP_CONTEXT] = %p, hfgContext=%p", nat->pContext[HSP_CONTEXT], nat->pContext[HSP_CONTEXT]->hfgContext);
        #if defined(EXT_DYNAMIC_LOADING)
        if( BT_STATUS_SUCCESS !=  nat->hfg_api.hfg_register(nat->pContext[HFP_CONTEXT], NULL, nat->hfgsock, /*nat->hfgservsock,*/ KAL_FALSE, nat->version) )
        #else        
        if( BT_STATUS_SUCCESS !=  btmtk_hfg_register(nat->pContext[HFP_CONTEXT], NULL, nat->hfgsock, /*nat->hfgservsock,*/ KAL_FALSE, nat->version) )
        #endif            
        {
...
static jboolean connectNative(JNIEnv *env, jobject object, jboolean wideband) 
{
#if defined(__BTMTK__) && defined(__BT_HFG_PROFILE__)
    BtStatus status = BT_STATUS_FAILED;
    ...
    if(nat)
    {
        ...
        {
            #if defined(EXT_DYNAMIC_LOADING)
            status = nat->hfg_api.hfg_create_audio_link(nat->activeContext, (wideband==JNI_TRUE)?1:0);
            #else
            status = btmtk_hfg_create_audio_link(nat->activeContext, (wideband==JNI_TRUE)?1:0);
            #endif
            if(status != BT_STATUS_PENDING)
            {
...
static JNINativeMethod sMethods[] = {
    ...
    /* Replacement of ScoSocket Interface */
    {"connectNative", "(Z)Z", (void *)connectNative},
