...
#define SDP_ATTR_REMOTE_AUDIO_VOL_CTL            0x0302
...
#define SDP_ATTR_SIZE_UUID_16				(2 + 2 + 1 + (BTMTK_SDP_UUID_16_BIT_SIZE + 1) * 3)
#define SDP_ATTR_SIZE_PROT 					(4 + 2 + 3 + 2 + 3 + 2 )	
#define SDP_ATTR_SIZE_PROFILE_VERSION	 	12
#define SDP_ATTR_SIZE_SUPPORTED_FEATURES	9
#define SDP_ATTR_SIZE_BROWSE_GROUP 			9
#define SDP_ATTR_SIZE_REMOTE_VOL_CTL		4

#define SDP_HF_DEFAULT_VERSION 					0x0105
#define SDP_HF_DEFAULT_FEATURES					0x0000
#define SDP_HS_DEFAULT_VERSION 					0x0100
#define SDP_HS_DEFAULT_REMOTE_VOL_CTL			0
...
static void btmtk_gap_send_name_change_event(native_data_t *nat, char *name, int namelen)
{
	...
	namelen = (namelen >= BTBM_ADP_MAX_NAME_LEN) ? (BTBM_ADP_MAX_NAME_LEN-1) : namelen;
...
bool btmtk_gap_set_scanable_mode(native_data_t *nat, btbm_scan_enable_type scanablemode)
{
       ...
       wait_response(nat->servsock, MSG_ID_BT_BM_WRITE_SCANENABLE_MODE_CNF, &ilm, BTMTK_MAX_STACK_TIMEOUT) > 0)
    {
        ...
        if (cnf_p->result == BTBM_ADP_SUCCESS)
        {
            ...
            {
            	nat->event_nat->host_cache.scan_mode = scanablemode;
...
int btmtk_util_get_raw_record_size(char *uuid128, const char *name)
{
	...
	if (btmtk_util_is_desired_uuid(uuid128, SC_HANDSFREE))
	{
		base_size += SDP_ATTR_SIZE_UUID_16 + 
						SDP_ATTR_SIZE_PROT + 
						SDP_ATTR_SIZE_PROFILE_VERSION + 
						SDP_ATTR_SIZE_SUPPORTED_FEATURES;
	}
	else if (btmtk_util_is_desired_uuid(uuid128, SC_HEADSET))
	{
		base_size += SDP_ATTR_SIZE_UUID_16 + 
						SDP_ATTR_SIZE_PROT + 
						SDP_ATTR_SIZE_PROFILE_VERSION + 
						SDP_ATTR_SIZE_REMOTE_VOL_CTL;
	}
	else
	{
...
BT_BOOL btmtk_sdp_util_add_assigned_service_class(SDP_Record *sdp_record, short uuid_16[], int uuid_num)
{	
	int size = 0;
	int index = 0;
	...
	size = (BTMTK_SDP_UUID_16_BIT_SIZE + 1)*uuid_num;
	...
	for (index = 0; index < uuid_num; index ++)
	{
		...
		/* 2 */ SDP_WRITE_16BIT(sdp_record->buffer, sdp_record->pos, uuid_16[index]);
	}	
	return TRUE;
}
...
BT_BOOL btmtk_sdp_util_add_supported_features(SDP_Record *sdp_record, short features)
{
	int size = 0;
	if (FALSE == btmtk_sdp_util_check_record(sdp_record, SDP_ATTR_SIZE_SUPPORTED_FEATURES))
	{
		return FALSE;
	}
	/* 2 */ SDP_WRITE_16BIT(sdp_record->buffer, sdp_record->pos, SDP_ATTR_SUPPORTED_FEATURES);
	/* 1 */ sdp_record->buffer[sdp_record->pos++] = btmtk_jsr82_int_get_data_element_header(BTMTK_SDP_ELEM_UNSIGNED_INT, 2);
	/* 2 */ SDP_WRITE_16BIT(sdp_record->buffer, sdp_record->pos, features);
	return TRUE;

}
BT_BOOL btmtk_sdp_util_add_remote_vol_ctl(SDP_Record *sdp_record, BT_BOOL supported)
{
	if (FALSE == btmtk_sdp_util_check_record(sdp_record, SDP_ATTR_SIZE_REMOTE_VOL_CTL))
	{
		return FALSE;
	}
	//Instance ID
	/* 2 */ SDP_WRITE_16BIT(sdp_record->buffer, sdp_record->pos, SDP_ATTR_REMOTE_AUDIO_VOL_CTL);
	/* 1 */ sdp_record->buffer[sdp_record->pos++] = btmtk_jsr82_int_get_data_element_header(BTMTK_SDP_ELEM_BOOL, 1);
	/* 1 */ SDP_WRITE_8BIT(sdp_record->buffer, sdp_record->pos, supported); 
	return TRUE;
...
BT_BOOL btmtk_jsr82_int_compose_hf_record(SDP_Record * sdp_record, const char *name, char *uuid, short channel)
{
	short version = 0x00;
	short features = 0x00;
	BT_CUST_ID custID;
	...
	ALOGI("btmtk_jsr82_int_compose_hf_record");

	
    custID = getCustID("hfsdp");    
	if (BT_CUST_UNKNOWN == custID)
	{
		version = SDP_HF_DEFAULT_VERSION;
		features = SDP_HF_DEFAULT_FEATURES;
	}
	else
	{    
    	getCustValue(custID, (unsigned int *)&hfSdpCfg);
		version = hfSdpCfg[2];
		features = hfSdpCfg[3];
	}

	uuids[0] = SC_HANDSFREE;
	uuids[1] = SC_GENERIC_AUDIO;


	if (FALSE == btmtk_sdp_util_add_assigned_service_class(sdp_record, uuids, 2))
	{
		ALOGI("[JSR82][SDP] error in service class");
		return FALSE;
	}

	if (FALSE == btmtk_sdp_util_add_common_protocol(sdp_record, channel))
	{
		ALOGI("[JSR82][SDP] error in protocol");
		return FALSE;
	}

	if (FALSE == btmtk_sdp_util_add_public_browse_group(sdp_record))
	{
		ALOGI("[JSR82][SDP] error in browse group");
		return FALSE;
	}
	...
	if (FALSE == btmtk_sdp_util_add_profile_descriptor(sdp_record, 
														...
														version))
	{
		ALOGI("[JSR82][SDP] error in profile descriptor");
		return FALSE;
	}

	if (FALSE == btmtk_sdp_util_add_service_name(sdp_record, name))
	{
		ALOGI("[JSR82][SDP] error in service name");
		return FALSE;
	}

	if (FALSE == btmtk_sdp_util_add_supported_features(sdp_record, features))
	{
		ALOGI("[JSR82][SDP] error in service name");
		return FALSE;
	}
    ...
    {
        ALOGE("error!!!! btmtk_jsr82_int_compose_common_record: size=%d >= malloc size=%d", sdp_record->pos, sdp_record->size);
    }
    ALOGI("btmtk_jsr82_int_compose_common_record: size=%d ", sdp_record->pos);
    return TRUE;     
 
}

BT_BOOL btmtk_jsr82_int_compose_hs_record(SDP_Record * sdp_record, const char *name, char *uuid, short channel)
{
	short version = 0x00;
	BT_BOOL vol_ctl_supported = 0x00;
	BT_CUST_ID custID;
	U16   *hsSdpCfg = NULL;
	short uuids[3];
	int uuids_num = 0;
	
	ALOGI("btmtk_jsr82_int_compose_hs_record");
	
    custID = getCustID("hssdp");    
	if (BT_CUST_UNKNOWN == custID)
	{
		version = SDP_HS_DEFAULT_VERSION;
		hsSdpCfg = SDP_HS_DEFAULT_REMOTE_VOL_CTL;
	}
	else
	{
    	getCustValue(custID, (unsigned int *)&hsSdpCfg);
		version = hsSdpCfg[2];
		vol_ctl_supported = hsSdpCfg[3];
	}
	if (SDP_HS_DEFAULT_VERSION == version)
	{
		uuids_num = 2;
		uuids[0] = SC_HEADSET;
		uuids[1] = SC_GENERIC_AUDIO;
	...
	else
	{
		...
		uuids[0] = SC_HEADSET;
		uuids[1] = SC_GENERIC_AUDIO;
		uuids[2] = SC_HEADSET_HS;
	}
	
	if (FALSE == btmtk_sdp_util_add_assigned_service_class(sdp_record, uuids, uuids_num))
	{
		ALOGI("[JSR82][SDP] error in service class");
		return FALSE;
	}
	...
	if (FALSE == btmtk_sdp_util_add_remote_vol_ctl(sdp_record, vol_ctl_supported))
	{
		ALOGI("[JSR82][SDP] error in remote volumne control");
		return FALSE;
	}
...
BT_BOOL btmtk_jsr82_int_compose_common_record(SDP_Record * sdp_record, const char *name, char *uuid, short channel)
{
	...
	ALOGI("btmtk_jsr82_int_compose_common_record");

	if (FALSE == btmtk_sdp_util_add_common_service_class(sdp_record, uuid))
	{
		ALOGE("[JSR82][SDP] error in service class");
		return FALSE;
	}

	if (FALSE == btmtk_sdp_util_add_common_protocol(sdp_record, channel))
	{
		ALOGE("[JSR82][SDP] error in protocol");
		return FALSE;
	}

	if (FALSE == btmtk_sdp_util_add_public_browse_group(sdp_record))
	{
		ALOGE("[JSR82][SDP] error in browse group");
		return FALSE;
	}

	if (FALSE == btmtk_sdp_util_add_service_name(sdp_record, name))
	{
		ALOGE("[JSR82][SDP] error in service name");
		return FALSE;
	}
...
int btmtk_jsr82_int_compose_record(char **buf, const char *name, char *uuid, short channel)
{
    ...
    ALOGI("btmtk_jsr82_int_compose_record: name=%s, channel=%d ", name, channel);
	...
	if (btmtk_util_is_desired_uuid(uuid, SC_HANDSFREE))
	{
		bRet = btmtk_jsr82_int_compose_hf_record(&record, name, uuid, channel);
	} 
	else if (btmtk_util_is_desired_uuid(uuid, SC_HEADSET))
	{
		bRet = btmtk_jsr82_int_compose_hs_record(&record, name, uuid, channel);
	}
	else
    {
...
int btmtk_register_reserved_uuid(native_data_t *nat, short uuid)
{
	char uuid_128[BTMTK_SDP_UUID_128_BIT_SIZE];
	char *rec;
	int rec_len;
	int handle = -1;
	short channel = 0x00;
	...
	if (FALSE == nat->cust_init)
	{
		initCust(NULL);
		nat->cust_init = TRUE;
	}

	if (BTMTK_SDP_UUID_HEADSET == uuid)
	{
		custID = getCustID("hssdp"); 
	    getCustValue(custID, (unsigned int *)&sdpCfg);   
		if (BT_CUST_UNKNOWN != custID && (sdpCfg[0] > 0))
		{ 
			channel = sdpCfg[1];
			btmtk_util_convert_uuid16_2_uuid128(uuid_128, BTMTK_SDP_UUID_HEADSET);
			rec_len = btmtk_jsr82_int_compose_record(&rec, "headset", uuid_128, channel);
			handle = btmtk_jsr82_create_record(nat, &rec, rec_len);
		}
	}
	else if (BTMTK_SDP_UUID_HF_HANDSFREE == uuid)
	{
		custID = getCustID("hfsdp"); 
	    getCustValue(custID, (unsigned int *)&sdpCfg);   
		if (BT_CUST_UNKNOWN != custID && (sdpCfg[0] > 0))
		{ 
			channel = sdpCfg[1];
			btmtk_util_convert_uuid16_2_uuid128(uuid_128, BTMTK_SDP_UUID_HF_HANDSFREE);
			rec_len = btmtk_jsr82_int_compose_record(&rec, "handsfree", uuid_128, channel);
			handle = btmtk_jsr82_create_record(nat, &rec, rec_len);
		}
	}
	else
	{
		ALOGE("[GAP] btmtk_register_reserved_uuid: not supported uuid = 0x%4x", uuid);
	}
	return handle;
}
bool btmtk_unregister_reserved_uuid(native_data_t *nat, int handle)
{
	return btmtk_jsr82_remove_record(nat, handle);
}
...
static jintArray addReservedServiceRecordsNative(JNIEnv *env, jobject object,
    ...
    jint* svc_classes = NULL;
	int svc_classes_len = 0;
	jint handles[BTBM_ADP_MAX_SDAP_UUID_NO];
	int handle_no = 0;
	int index = 0;
	int handle;
    jintArray handleArray = NULL;

	ALOGI("[GAP] addReservedServiceRecordsNative");

	nat = get_native_data(env, object);

	svc_classes = env->GetIntArrayElements(uuids, NULL);	
	if (!svc_classes) 
    return NULL;
	
	svc_classes_len = env->GetArrayLength(uuids);
	for (index = 0; index < svc_classes_len; index ++)
	{
		handle = btmtk_register_reserved_uuid(nat, (short)svc_classes[index]);
		if (handle > 0)
		{
			handles[handle_no++] = handle;
		}
	}	
	env->ReleaseIntArrayElements(uuids, svc_classes, 0);
	...
	if (handle_no > 0)
	{
		handleArray = env->NewIntArray(handle_no);
        if (handleArray) {
            env->SetIntArrayRegion(handleArray, 0, handle_no, handles);
        } else {
            ALOGE("Null array in addReservedServiceRecordsNative");
        }
	}
...
static jboolean removeReservedServiceRecordsNative(JNIEnv *env, jobject object,
    ...
    jint* uuid_handles = NULL;
	int uuid_handles_len = 0;
	int index = 0;

	ALOGI("[GAP] removeReservedServiceRecordsNative");

	nat = get_native_data(env, object);

	uuid_handles = env->GetIntArrayElements(handles, NULL);	
	if (!uuid_handles) 
		return JNI_FALSE;
	
	uuid_handles_len = env->GetArrayLength(handles);
	for (index = 0; index < uuid_handles_len; index ++)
	{
		btmtk_unregister_reserved_uuid(nat, uuid_handles[index]);
	}	
	env->ReleaseIntArrayElements(handles, uuid_handles, 0);
