...
#define SOCKET_ERROR_NO_EBADFD 77
#define SOCKET_ERROR_NO_EADDRINUSE 98
...
typedef struct
{
	...
	pthread_cond_t jsr82DiscCond;	// Condition variable for waiting
...
#define WRITE_DATA_TIMEOUT            60000
...
jboolean btmtk_jsr82_allocate_cntx(int *iIndex)
{
	...
	for (iCount = 0; iCount < JSR82_PORT_NUM; iCount++)
	{
		if (JNI_FALSE == g_jsr82MMIContext[iCount].inUse)
		{
			...
			g_jsr82MMIContext[iCount].forceToAttach = JNI_FALSE;
			*iIndex = iCount;
			return JNI_TRUE;
		}
	}
...
jbyte convert_ps_type(int type)
{
	jbyte ps_type = -1;
	...
	{
	case TYPE_RFCOMM:
		ps_type = JSR82_SESSION_PS_RFCOMM;
		break;
	case TYPE_L2CAP:
		ps_type = JSR82_SESSION_PS_L2CAP;
		break;
	case TYPE_SCO:
		ps_type = JSR82_SESSION_PS_SCO;
		break;
	default:
		XLOGE("[JSR82][JNI] Invalid type (%d) !!", type);
		ps_type = -1;
	}
	return ps_type;
...
int revert_ps_type(jbyte ps_type)
{
	int type = -1;
	...
	case JSR82_SESSION_PS_SCO:
		type = TYPE_SCO;
		break;
	default:
		XLOGE("[JSR82][JNI] Invalid ps type (%d) !!", ps_type);
		ps_type = -1;
	}
	return type;
...
int convert_error_no(int err_no)
{
	switch(err_no)
	{
	case JSR82_SESSION_STATUS_SUCCESS:
		return 0;

	case JSR82_SESSION_STATUS_FAILED:
	case JSR82_SESSION_STATUS_INVALID_PARAM:
		return -1;

	case JSR82_SESSION_STATUS_IN_USE:
		return SOCKET_ERROR_NO_EADDRINUSE;
...
static int initSocketNative(JNIEnv *env, jobject obj, jint type, jboolean auth, jboolean encrypt, jint port) 
{
	...
	ps_type = convert_ps_type(typeLocal);
	if (-1 == ps_type)
	{
		return -1;
	}
	...
	if (JNI_TRUE == bAllocateResult)
	{
		...
		g_jsr82MMIContext[iCntxNew].channelNumber = portLocal;
		pthread_cond_init(&g_jsr82MMIContext[iCntxNew].jsr82ConnectCond, NULL);
		pthread_cond_init(&g_jsr82MMIContext[iCntxNew].jsr82DiscCond, NULL);
	...
	//	g_jsr82MMIContext[iCntxNew].jsr82RegisterCond = PTHREAD_COND_INITIALIZER;
		...
		g_jsr82MMIContext[iCntxNew].ps_type = ps_type;
		...
		if (0 == g_jsr82MMIContext[iCntxNew].jsr82DiscCond.value)
		{
			pthread_mutex_lock(&(nat->thread_mutex));
			if( 0 != pthread_cond_init(&(g_jsr82MMIContext[iCntxNew].jsr82DiscCond), NULL))
			{
				pthread_mutex_unlock(&(nat->thread_mutex));
				XLOGE("[JSR82][JNI] pthread_cond_init failed <Connect>");
				return -1;
			...
			{
				pthread_mutex_unlock(&(nat->thread_mutex));
				XLOGI("[JSR82][JNI] jsr82DiscCond (%d) initialization success ", iCntxNew);
			}
		}
...
static jint connectNative(JNIEnv *env, jobject obj, jint fdHandle, jstring sAddr, jint channelNumber) 
{
	...
	if (JNI_TRUE == bSearchResult)
	{
		...
		bConnectResult = btmtk_jsr82_session_connect_req(g_jsr82MMIContext[iCntxIndex].fd, 
			aAddr, g_jsr82MMIContext[iCntxIndex].ps_type, g_jsr82MMIContext[iCntxIndex].channelNumber, 
		...
		else
		{
			...
			iResult = pthread_cond_wait(&(g_jsr82MMIContext[iCntxIndex].jsr82ConnectCond), &(nat->thread_mutex));
			if (JNI_TRUE == g_jsr82MMIContext[iCntxIndex].bAborting ||
				0 != iResult ||
				JNI_TRUE != g_jsr82MMIContext[iCntxIndex].bConnected)
			{
				XLOGI("[JSR82][JNI] connectNative failed");
				iResult = -1;
			}	
			else
			{
	...
	else
	{
		...
		iResult = SOCKET_ERROR_NO_EBADFD;
	}
	
	XLOGI("[JSR82][JNI] connectNative [%d]---.", iResult);
	return iResult;
#endif
	jniThrowIOException(env, ENOSYS);
	return -1;
...
static int bindListenNative(JNIEnv *env, jobject obj, jint fdHandle) 
{
	...
	if (JNI_TRUE == bSearchResult)
	{
		...
		{
			if (JNI_TRUE == g_jsr82MMIContext[iCntxIndex].forceToAttach)
			{
			...
			{
				bRegisterResult = btmtk_jsr82_session_service_registration_use_fixed_chnl_num(
							g_jsr82MMIContext[iCntxIndex].ps_type, 
							mtu, securityMask, g_jsr82MMIContext[iCntxIndex].fd, 
							g_jsr82MMIContext[iCntxIndex].channelNumber);
		...
		{
			g_jsr82MMIContext[iCntxIndex].index = -1;
			g_jsr82MMIContext[iCntxIndex].err_no = JSR82_SESSION_STATUS_INVALID_PARAM;
		}
		else
		{
			...
			iResult = pthread_cond_timedwait(&(g_jsr82MMIContext[iCntxIndex].jsr82RegisterCond), &(nat->thread_mutex), &nextTs);
			
			if (JNI_TRUE == g_jsr82MMIContext[iCntxIndex].bAborting ||
				0 != iResult)
			{
				XLOGI("[JSR82][JNI] bindListenNative: This port is aborting. or Wait result is not correct. %d, %s", errno, strerror(errno));
		...
		if (0 <= g_jsr82MMIContext[iCntxIndex].index)
		{
		...
		{		
			iResult = convert_error_no(g_jsr82MMIContext[iCntxIndex].err_no);
		}
		pthread_mutex_unlock(&(nat->thread_mutex));
	...
	XLOGI("[JSR82][JNI] bindListenNative [%d]---.", iResult);
	return iResult;
#endif
	jniThrowIOException(env, ENOSYS);
	return -1;
...
static int acceptNative(JNIEnv *env, jobject obj, int timeout, jint fdHandle) 
{
	...
	if (JNI_TRUE == bSearchResult)
	{
		...
		fdHandleNew = initSocketNative(env, obj, revert_ps_type(g_jsr82MMIContext[iCntxIndex].ps_type), g_jsr82MMIContext[iCntxIndex].bAuthentication, 
		...
		if (JNI_TRUE == bSearchResult)
		{
			...
			g_jsr82MMIContext[iCntxIndexNew].forceToAttach = JNI_TRUE;
		}
		else
		{
...
static jint readNative(JNIEnv *env, jobject obj, jbyteArray jb, jint offset, jint length, jint fdHandle) 
{
	...
	if (bufPtr == NULL)
	{
		XLOGE("[JSR82][JNI] ByteArray pointer bufPtr is NULL or length is smaller than 0.");
		iReadResult = -1;
		goto exit;
	}
	...
	if (JNI_TRUE == bSearchResult)
	{
		...
		while (TRUE)
		{
			...
			if (JNI_TRUE == g_jsr82MMIContext[iCntxIndex].bAborting ||
				...
				0 != iWaitResult)
			{
				XLOGI("[JSR82][JNI] readNative error: aborting[%d], bConnected[%d], iWaitResult[%d]",
														g_jsr82MMIContext[iCntxIndex].bAborting,
														g_jsr82MMIContext[iCntxIndex].bConnected,
														iWaitResult);					
				iReadResult = -1;
				break;
			}	
...
exit:
	XLOGI("[JSR82][JNI] readNative [%d]--- ", iReadResult);
	if (iReadResult < 0)
	{
		iReadResult = -1;
	}
...
exit:
	XLOGI("[JSR82][JNI] writeNative [%d]---." ,totalWriteResult);
	env->ReleaseByteArrayElements(jb, bufPtr, JNI_ABORT);
...
static int abortNative(JNIEnv *env, jobject obj, jint fdHandle) 
{
	...
	if (JNI_TRUE == bSearchResult)
	{
		...
		if (JNI_TRUE == g_jsr82MMIContext[iCntxIndex].bConnected)
		{
			...
			pthread_mutex_lock(&(nat->thread_mutex));
			iWaitResult = pthread_cond_wait(&(g_jsr82MMIContext[iCntxIndex].jsr82DiscCond), &(nat->thread_mutex));
			pthread_mutex_unlock(&(nat->thread_mutex));			
		}
...
//	bSearchResult = btmtk_jsr82_search_cntx_by_fd(fdHandle, &iCntxIndex);
	...
	if (0 <= iCntxIndex)
	{
		...
		pthread_cond_destroy(&(g_jsr82MMIContext[iCntxIndex].jsr82DiscCond));
...
void btmtk_jsr82_android_cb_enable_service_cnf(void *parms)
{
	...
	if (JNI_TRUE == bSearchResult)
	{
		...
		if (JSR82_SESSION_STATUS_SUCCESS == msg->result)
		{
			pthread_mutex_lock(&(nat->thread_mutex));
		...
		else
		{
			...
			pthread_mutex_lock(&(nat->thread_mutex));
			g_jsr82MMIContext[iCntxIndex].index = -1;
			g_jsr82MMIContext[iCntxIndex].err_no = msg->result;
			g_jsr82MMIContext[iCntxIndex].channelNumber = 0;
			g_jsr82MMIContext[iCntxIndex].bRegistering = JNI_FALSE;
			pthread_cond_signal(&(g_jsr82MMIContext[iCntxIndex].jsr82RegisterCond));
			pthread_mutex_unlock(&(nat->thread_mutex));
			return ;
...
void btmtk_jsr82_android_cb_connect_ind(void *parms)
{
	...
	if (JNI_TRUE == bSearchResult)
	{
		if (JSR82_SESSION_STATUS_SUCCESS == msg->rsp_result)
		{
			pthread_mutex_lock(&(nat->thread_mutex));
...
void btmtk_jsr82_android_cb_disconnect_ind(void *parms)
{
	...
	if (JNI_TRUE == bSearchResult)
	{
		...
		pthread_cond_signal(&(g_jsr82MMIContext[iCntxIndex].jsr82DiscCond));
		pthread_cond_signal(&(g_jsr82MMIContext[iCntxIndex].jsr82ReadCond));
		pthread_cond_signal(&(g_jsr82MMIContext[iCntxIndex].jsr82WriteCond));
		...
		{			
			...
			{
				g_jsr82MMIContext[iCntxIndex].index = 0xFF;				
			}
			pthread_mutex_unlock(&(nat->thread_mutex));
		}
		// Check if it is necessary to DISABLE this port: Server case
		else if (JNI_TRUE == g_jsr82MMIContext[iCntxIndex].bServer)
		{
