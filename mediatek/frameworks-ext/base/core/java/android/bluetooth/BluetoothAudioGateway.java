...
public final class BluetoothAudioGateway /*extends HeadsetBase*/ {
    ...
    public BluetoothAudioGateway(PowerManager pm, BluetoothAdapter adapter, int handsfreeAgRfcommChannel,
        ...
        if(FeatureOption.MTK_HFP1_6_SUPPORT == true){
            mHfpVersion = 0x0106;
        }
    ...
    */
    ...
    public synchronized boolean connect(boolean wideband){
        log("[API] connect(wideband="+String.valueOf(wideband)+")");
        if(connectNative(wideband) == true)
        {
            mOutgoingSCO = true;
            return true;
        }
    ...
    /***************************************************************/
    private native boolean connectNative(boolean bWideBand);
