...
#if 0
 static int get_v_chr(void)
 {
	 int vchr_val=0;
	 vchr_val = PMIC_IMM_GetOneChannelValue(VCHARGER_CHANNEL_NUMBER,5,1);
	 vchr_val = (((330+39)*100*vchr_val)/39)/100;
	 return vchr_val;
 }
 
 static int get_v_bat_sen(void)
 {
	 int v_bat_sen_val=0;
	 v_bat_sen_val = PMIC_IMM_GetOneChannelValue(VBAT_CHANNEL_NUMBER,1,1);
	 return v_bat_sen_val;
 }
 
 static int get_v_i_sen(void)
 {
	 int v_i_sen_val=0;
	 v_i_sen_val = PMIC_IMM_GetOneChannelValue(ISENSE_CHANNEL_NUMBER,1,1);
	 return v_i_sen_val;
 }

 static int get_i_chr(void)
 {
	 int v_i_sen_val=0;
	 int v_bat_sen_val=0;
 	 int ICharging=0;
	 ...
	 ICharging = (v_i_sen_val-v_bat_sen_val)*1000/CUST_R_SENSE;
	 return ICharging;
 }
#endif 
