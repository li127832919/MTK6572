...
//kal_uint32 g_bat_time_before_sleep=0;
...
{
    kal_uint8 ktime_1sec_max_count = 3;
	static kal_uint8 ktime_1sec_count=0;
    ...
    while (1) {               
		...
		{
			ktime_1sec_count++;
			ktime = ktime_set(1, 0);  // 1s, 10* 1000 ms
		}
		else
		{
...
//	g_bat_time_before_sleep = rtc_read_hw_time(); 
	getrawmonotonic(&g_bat_time_before_sleep);
...
static int battery_resume(struct platform_device *dev)
{
	...
	//kal_uint32 bat_time_after_sleep;

	//bat_time_after_sleep = rtc_read_hw_time(); 
	struct timespec bat_time_after_sleep;

	getrawmonotonic(&bat_time_after_sleep);
