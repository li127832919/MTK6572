...
MTK_WCN_BOOL wmt_cdev_rstmsg_snd(ENUM_WMTRSTMSG_TYPE_T msg,ENUM_WMTRSTRET_TYPE_T ret){
	...
	if((WMTRSTMSG_RESET_START == msg) || 
		((WMTRSTMSG_RESET_END == msg) && (WMTRSTRET_SUCCESS == ret)))
	{
	    for(i = 0 ; i <= WMTDRV_TYPE_WIFI; i++){
	...
	}else if((WMTRSTMSG_RESET_END == msg) && (WMTRSTRET_FAIL == ret))
	{
		ENUM_WMTRSTMSG_TYPE_T msgforwifi = WMTRSTMSG_RESET_FAIL;
		
		for(i = 0 ; i <= WMTDRV_TYPE_GPS; i++){
	        //<1> check if reset callback is registered
	        if(pDevWmt->rFdrvCb.fDrvRst[i]){
	            //<2> send the msg to this subfucntion
	            /*src, dst, msg_type, msg_data, msg_size*/
		...
		if(pDevWmt->rFdrvCb.fDrvRst[WMTDRV_TYPE_WIFI])
		{
			WMT_INFO_FUNC("notify wifi that whole chip reset fail\n");
			pDevWmt->rFdrvCb.fDrvRst[WMTDRV_TYPE_WIFI](WMTDRV_TYPE_WMT, 
				WMTDRV_TYPE_WIFI, WMTMSG_TYPE_RESET, &msgforwifi, sizeof(ENUM_WMTRSTMSG_TYPE_T));
		}
	}
    return MTK_WCN_BOOL_TRUE;
}
...
MTK_WCN_BOOL wmt_lib_hw_power_off(VOID)
{
    P_OSAL_OP pOp;
    MTK_WCN_BOOL bRet;
    P_OSAL_SIGNAL pSignal;
    ...
    WMT_DBG_FUNC("call WMT_OPID_PWR_OFF\n");
    ...
    bRet = wmt_lib_put_act_op(pOp);
    if (MTK_WCN_BOOL_FALSE == bRet) {
        WMT_WARN_FUNC("WMT_OPID_PWR_OFF failed\n");
        return MTK_WCN_BOOL_FALSE;
    }
    else {
        WMT_DBG_FUNC("OPID(WMT_OPID_PWR_OFF)ok\n");
    }
    return MTK_WCN_BOOL_TRUE;
}
...
ENUM_WMTRSTRET_TYPE_T wmt_lib_cmb_rst(ENUM_WMTRSTSRC_TYPE_T src){
    ...
    bRet = wmt_cdev_rstmsg_snd(WMTRSTMSG_RESET_START,WMTRSTRET_MAX);
    ...
    if(bRet == MTK_WCN_BOOL_FALSE){
        ...
        retval = WMTRSTRET_FAIL;
		wmt_cdev_rstmsg_snd(WMTRSTMSG_RESET_END,WMTRSTRET_FAIL);
        goto rstDone;
    } else {
    ...
    bRet = wmt_cdev_rstmsg_snd(WMTRSTMSG_RESET_END,WMTRSTRET_SUCCESS);
    if(bRet == MTK_WCN_BOOL_FALSE){
        WMT_ERR_FUNC("[whole chip reset] fail at wmt_lib_rstmsg_snd! \n");
        retval = WMTRSTRET_FAIL;
        goto rstDone;
...
rstDone:
	...
	if(retval == WMTRSTRET_FAIL)
	{
		wmt_lib_hw_power_off();
	}
    return retval;
}
